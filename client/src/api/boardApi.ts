...
export async function addNewBoard(board: { id: string; name: string }): Promise<Board> {
    const res = await fetch(BASE_URL, {
        method: 'POST',
...
export async function updateBoard(boardId: string, update: Partial<Board>): Promise<Board> {
  const res = await fetch(`${BASE_URL}/${boardId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
